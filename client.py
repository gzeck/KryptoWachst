...
class Client(object):
  ...
  def get_buy_price(self, **params):
    ...
    response = self._get('v2', 'prices', currency_pair, 'buy', data=params)  ## GZ - Added currency_pair
    return self._make_api_object(response, APIObject)

  def get_sell_price(self, **params):
    """https://developers.coinbase.com/api/v2#get-sell-price"""
    currency_pair = params['currency_pair']
    response = self._get('v2', 'prices', currency_pair, 'sell', data=params) ## GZ - Added currency_pair
    return self._make_api_object(response, APIObject)

  def get_spot_price(self, **params):
    """https://developers.coinbase.com/api/v2#get-spot-price"""
    currency_pair = params['currency_pair']
    response = self._get('v2', 'prices', currency_pair, 'spot', data=params) ## GZ - Added currency_pair
    return self._make_api_object(response, APIObject)

  def get_historic_prices(self, **params):
    """https://developers.coinbase.com/api/v2#get-historic-prices"""
    currency_pair = params['currency_pair']
    response = self._get('v2', 'prices', currency_pair, 'historic', data=params) ## GZ - Added currency_pair
    return self._make_api_object(response, APIObject)
